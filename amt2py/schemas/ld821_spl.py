...
from amt2py.schemas.monitoring_site import (
    infer_monitoring_site_id,
    path_basename_chain,
)
...
def infer_site_id_from_spl_csv_path(csv_path: str) -> str:
    """Site ID hint when browsing combined (or other) SPL CSV for NVSPL."""
    sources = []
    prefix = parse_site_from_combined_spl_filename(csv_path)
    if prefix:
        sources.append(prefix)
    sources.extend(path_basename_chain(csv_path))
    return infer_monitoring_site_id(*sources)
...
def infer_spl_gui_defaults(csv_path):
    ...
    site = infer_site_id_from_spl_csv_path(csv_path)
    if site:
        result["SITE_ID"] = site
    return result
